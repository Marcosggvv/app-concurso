...
# =========================================================
# CACHE E BUSCA PARALELA OTIMIZADA
# =========================================================
...
def buscar_ddg(queries: List[str], max_res: int = 6, max_chars: int = 8000, cache_prefix: str = "") -> str:
    ...
    def fetch_single_query(q):
        ...
        try:
            ...
            texto_limpo = sanitize_text(texto, max_chars)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(queries), 5)) as executor:
        resultados_paralelos = list(executor.map(fetch_single_query, queries))
    
    resultados = [r for r in resultados_paralelos if r]
...
# =========================================================
# LLM CALL COM EXTRATOR BLINDADO
# =========================================================
...
def gerar_questoes(qtd, origem, banca, cargo, concurso, materia, tema, usar_web, modelo_escolhido):
    ...
    try:
        dados = json.loads(conteudo_limpo.replace("```json", "").replace("```", "").strip())
    except Exception as e:
        raise RuntimeError(f"Falha ao interpretar a resposta da IA como JSON: {e}")
...
with st.container(border=True):
    ...
    with c3col:
        tipo = st.selectbox("Origem", ["🧠 Inéditas IA", "🌐 Questões Reais", "📂 Revisão (Focada em Erros)"])
    with c4col:
    ...
    if st.button("🚀 Forjar Bateria", type="primary", use_container_width=True):
        ...
        # REVISÃO INTELIGENTE: Puxa questões do banco local priorizando as que o usuário errou
        if "Revisão" in tipo:
            st.info("🔄 Resgatando questões (priorizando erros passados)...")
            query_revisao = """
                SELECT q.id 
                FROM questoes q
...
# =========================================================
# CADERNO DE PROVA (TODAS NA MESMA TELA)
# =========================================================
